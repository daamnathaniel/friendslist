class ConnectionsController < ApplicationController
  ...
  before_action :resync_connections, only: :index
  # GET /connections or /connections.json
  def index
    @connected_users = current_user.connected_users.page(params[:page])
  end
  ...
  private
    ...
    def resync_connections
      if current_user.connections_outdated?
        new_connections = Connection.connections_for(current_user)
        current_user.connected_users.replace(new_connections)
      end
    end
